...
import { Send, ShieldAlert, Lock, Loader2, Clock } from 'lucide-react';
...
export default function ChatThreadPage() {
  ...
  const meta = React.useMemo(() => {
    if (isAdvocate) {
      ...
      return c ? { name: c.citizen_name || 'Citizen', status: c.status } : null;
    }
    const c = (citizenQ.data ?? []).find((x) => x.consultationId === consultationId);
    return c ? { name: c.advocateName || 'Advocate', status: c.status } : null;
  ...
  return (
    <div className="flex h-[calc(100svh-4rem)] flex-col">
      ...
      <div className="border-t border-border px-4 py-3">
        {closed ? (
          <p className="flex items-center justify-center gap-2 rounded-lg bg-muted/50 px-4 py-3 text-sm text-muted-foreground">
            <Lock className="size-4" /> {t('chat.closedNotice')}
          </p>
        ) : (
          <>
