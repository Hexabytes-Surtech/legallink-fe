...
import {
  USE_MOCK, mockDelay,
  MOCK_MATTER, MOCK_ADVOCATES,
  type Matter, type Advocate, type Consultation,
} from '@/lib/mock-data';
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      if (USE_MOCK) {
        ...
        const res = await apiClient(`/matter/${id}/documents`, {
          method: 'POST',
          formData,
        });
        if (res.success) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-cream)' }}>
        ...
        <div style={{ textAlign: 'center', padding: '6rem 1.5rem' }}>
          ...
          <Link href="/app/intake" className="btn btn-primary" style={{ display: 'inline-flex', marginTop: '1rem' }}>
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--color-cream)' }}>
      ...
      {showOTP && (
        <OTPModal
          ...
          contextMessage={selectedAdvocate
            ? `To request a consultation with ${selectedAdvocate.name}, please sign in or create a free account.`
            : undefined}
          onSuccess={() => {
            if (selectedAdvocate) submitConsultation(selectedAdvocate.id);
          }}
          redirectTo={`/app/matter/${id}`}
        />
      )}
      ...
      <main style={{ flex: 1, maxWidth: '960px', margin: '0 auto', width: '100%', padding: '2.5rem 1.25rem 4rem' }}>
        ...
        {consultationStatus && (
          <div style={{
            background: consultationStatus.status === 'accepted'
              ? 'rgba(16,185,129,0.08)' : 'rgba(201,168,76,0.08)',
            border: `1px solid ${consultationStatus.status === 'accepted' ? 'rgba(16,185,129,0.3)' : 'rgba(201,168,76,0.3)'}`,
            borderRadius: '0.875rem',
            padding: '1rem 1.25rem',
            marginBottom: '1.5rem',
            display: 'flex',
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, color: 'var(--color-navy)' }}>
                {consultationStatus.status === 'accepted'
                  ? t('chat.consultation.accepted')
                  : t('chat.consultation.waiting')}
              </div>
            </div>
            {consultationStatus.status === 'accepted' && (
              <Link
                href={`/app/chat/${consultationStatus.id}`}
        ...
        <section style={{ marginBottom: '2rem', animation: 'fadeIn 0.5s ease' }}>
          <div className="card" style={{ padding: '1.75rem 2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
              ...
              <h2 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'var(--color-navy)', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
                {t('matter.classification')}
              </h2>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
              {matter.classification?.matterType && (
                <div>
                  <div style={{ fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#9CA3AF', marginBottom: '4px' }}>
                    {t('matter.type')}
                  </div>
          ...
          <div className="card" style={{ overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <h2 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'white', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
                  {t('matter.aiResponse')}
                </h2>
              </div>
              {/* Tab switcher */}
                ...
                {(['english', 'bengali'] as ResponseTab[]).map(tabOpt => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      transition: 'all 0.2s',
                      background: tab === tabOpt ? '#C9A84C' : 'transparent',
                      color: tab === tabOpt ? '#0D1B2A' : 'rgba(255,255,255,0.6)',
            ...
            <div style={{ padding: '2rem' }}>
              ...
              {matter.citations?.length > 0 && (
                <div style={{ marginBottom: '1.5rem' }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#9CA3AF', marginBottom: '0.625rem' }}>
                    {t('matter.citations')}
                  </div>
        ...
        {!isAuthenticated && (
          <section style={{ marginBottom: '2rem', animation: 'slideInUp 0.5s ease 0.2s both' }}>
            <div style={{
              ...
              <div>
                <h3 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'white', marginBottom: '0.375rem', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
                  {t('matter.cta.title')}
                </h3>
                <p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.55)', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
                  {t('matter.cta.subtitle')}
                </p>
              </div>
              <button onClick={() => setShowOTP(true)} className="btn btn-primary" style={{ whiteSpace: 'nowrap' }}>
                {t('matter.cta.button')}
              </button>
            </div>
          </section>
        ...
        {/* ── Section 3: Document Upload (authenticated) ────────── */}
        {isAuthenticated && (
          <section style={{ marginBottom: '2rem', animation: 'slideInUp 0.5s ease 0.25s both' }}>
            <div className="card" style={{ padding: '1.75rem 2rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                <h3 style={{ fontSize: '1.0625rem', fontWeight: 700, color: 'var(--color-navy)', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
                  {t('matter.documents')}
                </h3>
                <button
              ...
              {uploadedDocs.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--color-gray-400)', fontSize: '0.9rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  {uploadedDocs.map((doc, i) => (
                    <div key={i} style={{
                      ...
                      <span style={{ fontSize: '1.25rem' }}>📄</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 500, fontSize: '0.9rem', color: 'var(--color-navy)' }}>{doc.name}</div>
        ...
        {/* ── Section 4: Matching Advocates ─────────────────────── */}
        <section style={{ animation: 'slideInUp 0.5s ease 0.3s both' }}>
          <div style={{ marginBottom: '1.5rem' }}>
            <h2 className="text-title" style={{ color: 'var(--color-navy)', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
              {t('matter.advocates.title')}
            </h2>
            <p style={{ color: 'var(--color-gray-500)', fontSize: '0.9375rem', marginTop: '4px', fontFamily: language === 'bn' ? 'var(--font-bangla)' : 'inherit' }}>
              {t('matter.advocates.subtitle')}
            </p>
          </div>

          {advocates.length === 0 ? (
            <div className="card" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--color-gray-400)' }}>
